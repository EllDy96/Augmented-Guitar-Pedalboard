...
class NatNetClient:
    ...
    def __unpackMocapData( self, data ):
        ...
        if( ( self.__natNetStreamVersion[0] == 2 and self.__natNetStreamVersion[1] >= 7 ) or self.__natNetStreamVersion[0] > 2 ):
            ...
            self.timestamp, = FloatValue.unpack( data[offset:offset+4] )
            offset += 4
        ...
        if self.newFrameListener is not None:
            self.newFrameListener( frameNumber, markerSetCount, unlabeledMarkersCount, rigidBodyCount, skeletonCount,
                                  labeledMarkerCount, timecode, timecodeSub, self.timestamp, isRecording, trackedModelsChanged )

        # Send rigid body list and timestamp
        if self.rigidBodyListListener is not None:
            self.rigidBodyListListener( self.rigidBodyList, self.timestamp )
    ...
    def __unpackRigidBodyDescription( self, data ):
        ...
        self.timestamp = Vector3.unpack( data[offset:offset+12] )
        offset += 12
    ...
    def __dataThreadFunction( self, sock,event,check_error):
        sock.settimeout(0.01)
        #self.count=0
        #self.count_prima=0
        self.count_process_message=0
        #event.wait()
        if(not check_error): 
            event.wait()
            self.tempo_inizio=datetime.datetime.now()
            print(f'start optitrack acquisition {self.tempo_inizio}')
            self.tempo_precedente=self.tempo_inizio
            self.sendCommand( self.NAT_REQUEST, "StartRecording", self.commandSocket, (self.serverIPAddress, self.commandPort) )
            #tempo_p=self.tempo_inizio
            while self.running:
                #print(f'prima {self.count_prima}')
                #self.count_prima=self.count_prima+1
                # Block for input
                try:
                    data, addr = sock.recvfrom( 32768 ) # 32k byte buffer size
                    #print(datetime.datetime.now()-tempo_p)
                    #tempo_p=datetime.datetime.now()
                    #print(f'recive: {datetime.datetime.now()}')
                    if( len( data ) >= 4):
                        self.__processMessage( data )
                        #print(f'tempo attuale {datetime.datetime.now().time()}')
                        #print(self.timestamp) #riferito a quando si apre il motive
                        #print(f'differenza {datetime.datetime.now()-self.tempo_inizio}')
                        #print(f'dopo {self.count}')
                        #self.count=self.count+1
                        #print(self.rigidBodyList)
                        #self.filewriter_opti.writerow([self.rigidBodyList[0][1][0],self.rigidBodyList[0][1][1],self.rigidBodyList[0][1][2],datetime.datetime.now()-self.tempo_inizio])
                        #print(f'Pos X: {self.rigidBodyList[0][1][0]}')
                        #print(f'Pos Y: {self.rigidBodyList[0][1][1]}')
                        #print(f'Pos Z: {self.rigidBodyList[0][1][2]}')
                except socket.timeout:
                    pass
        else:
            self.stop(check_error)
    ...
    def __processMessage( self, data ):
        ...
        #print(f'tempo {datetime.datetime.now()-self.tempo_precedente}')
        #self.tempo_precedente=datetime.datetime.now()
        #print(f'process message count {self.count_process_message}')
        if(self.count_process_message==255): #il primo campione della registrazione parte a circa 255 cicli
            print(f'tempo primo campione optitrack {datetime.datetime.now()}')
        self.count_process_message=self.count_process_message+1
    ...
    def run(self,event,check_error):
        ...
        self.dataThread = Thread( target = self.__dataThreadFunction, args = (self.dataSocket,event,check_error ))
    ...
    def stop(self,check_error):
        self.running = False 
        self.sendCommand(self.NAT_REQUEST, "StopRecording", self.commandSocket, (self.serverIPAddress, self.commandPort) )
        if(not check_error): 
            self.dataThread.join() 
            print(f'stop optitrack acquisition {datetime.datetime.now()}  durata: {datetime.datetime.now()-self.tempo_inizio}')
        else:
            print(f'stop optitrack acquisition {datetime.datetime.now()}')
