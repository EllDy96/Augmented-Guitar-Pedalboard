...
class Application:
    ...
    def __init__(self,master): #il parametro master è la finestra generata con tkinter        
        ...
        self.lowcut = 20.0 #inizializzazione frequenza taglia basso
        ...
        self.default_low = 20.0 #inizializzazione frequenza taglia basso utilizzata per ripristinare i valori di default
        ...
        self.test_lowcut = 20.0 #inizializzazione frequenza taglia basso di test
    ...
    def single_channel(self,tab_id=''): #funzione che viene chiamata alla pressione di un tasto del menù 'muscles' (se la tab_editor non ha il focus)
        ...
        if not self.check_tab_channel[ID_channel]: #se non è stata ancora creata la tab per il muscolo con identificativo 'self.ID_channel_under_investigation'
            ...
            self.fig_coerence[ID_channel].canvas.set_window_title('COERENZA SEGNALI')
            ...
            self.fig_features[ID_channel].canvas.set_window_title('EMG AND FEATURES')
    ...
    def all_channels(self):
        ...
        if not self.check_tab_all_channels: #se la tab che mostra i plot alla pressione del tasto 'all channels' non è presente la creo
            ...
            if not self.number_of_channels==1:
                ...
                self.fig_all_filtered.canvas.set_window_title('EMG FILTERED')
                ...
                self.fig_all_spectrogram.canvas.set_window_title('SPETTROGRAMMI')
                ...
                self.fig_all_PSD.canvas.set_window_title('PSD')
                ...
                self.fig_all_MNF.canvas.set_window_title('MEAN FREQUENCY')
                ...
                self.fig_all_RMS.canvas.set_window_title('RMS ALL CHANNELS')
    ...
    def fatigue(self):
        ...
        if not self.check_tab_fatigue: #se la tab non è presente
            ...
            for i in range(self.number_of_channels): #ciclo per calcolare l'affaticamento su tutti i muscoli                    
                ...
                fig = plt.gcf()
                fig.canvas.set_window_title('FATIGUE ANALYSIS')
