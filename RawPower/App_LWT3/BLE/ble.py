...
def BLE_acquiring(BLE_MAC,start_event,stop_event,path_directory):
    ...
    """
    from bleak import BleakClient #Bleak is an acronym for Bluetooth Low Energy platform Agnostic Klient.
    #alzo priorità del processo
    p=psutil.Process()
    p.nice(psutil.REALTIME_PRIORITY_CLASS)
    name_BLE= "Music Sensor LWT3- d8d2"
    UUID_BLE= "6E400001-B5A3-F393-E0A9-E50E24DCCA9E"
    # hardcodiamo il UUID della sceda BLE con cui stiamo lavorando, se vuoi tornare indietro cancella UUID specifico
    async def run(address, loop, UUID = "6E400001-B5A3-F393-E0A9-E50E24DCCA9E" , name= "Music Sensor LWT3- d8d2"):
    ...
    if not BLE_MAC == []: #check se esistono dei BLE address
        ...
        *(run(address,loop,UUID,name) for address,UUID,name in zip(addresses,UUIDs,names))
        )
